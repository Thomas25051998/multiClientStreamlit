...
def filter_rieker_database(color, shoe_type, gender, season, material):
 ...
 #       conditions.append(Rieker_Database["Verschluss"] == closure)
    ...
    if conditions:
        ...
        print("Keine Filterkriterien angegeben.")
...
while True:
    ...
    if user_input.lower() == "exit" or user_input.lower() == "quit":
        ...
        chatVerlauf_UserInteraction=[{
        "role": "system",
           "content": f"You are a polite and helpful assistant who should help the user find the right Rieker shoes. For your first Messager you have none of the following Informations. You get a JSON file {JSON_Data} with the following variables Color, Shoe Type, Gender, Season, Material and Size. These are filters with which we want to find the right shoe for the customer. If a variable could not yet be recognized from the User_Input, there is a '' in the JSON file. If this is the case, explicitly ask the user again for the filter. You should mention the current amount of the filtered shoes matching to the User Input, given with this variable {lengthOfFilteredDatabase}. Mention {lengthOfFilteredDatabase} in the Answer. If {lengthOfFilteredDatabase} is greater than 10 then ask again for more information. But if {lengthOfFilteredDatabase} <= then give a description of the 10 filtered shoes. I will give you a pandas dataframe { filtered_DF} with which you can find the necessary information about the filtered shoes and describe those as deatiled as you can with the given Information to the User. Please describe the shoes in a continuous text and not in embroidery dots. "
        }]
